...
def process_configs(target, arg_parser):
    ...
    while len(gpu_queue)==0:
        ...
        if len(gpu_queue)==0:
            ...
            time.sleep(60)
    print("Using GPU devices: ",gpu_queue)
...
def _yield_configs(arg_parser, args, verbose=True):
    ...
    if args.config:
        ...
        for run_repeat, run_config in config:
            ...
            run_args_list = []
            # batch eval
            if run_args.tokenizer_path=="batch_eval_flag":
                ...
                for dirpath,dirnames,filenames in sorted(os.walk(save_path),key=lambda x:x[0]):
                    if dirpath.endswith("final_model"):
                        dataset_name=re.match(".*?/.*?/.*?/(.*?)_",dirpath).group(1)
                        # print(dirpath)
                        ...
                        if dataset_name=="ade" or dataset_name=="ace04":
                            ...
                            run_args.dataset_path ="data/datasets2/"+dataset_name+"/"+dataset_name2+"_test.json"
                        else:
                            run_args.label= dataset_name+"_eval"
                            run_args.dataset_path ="data/datasets2/"+dataset_name+"/"+dataset_name+"_test.json"

                        run_args.model_path=dirpath
                        run_args.tokenizer_path=dirpath
                        run_args.types_path = "data/datasets2/"+dataset_name+"/"+dataset_name+"_types.json"

                        args_path=run_args.log_path+"/"+exp_label+"/"+exp_time+"/args.json"
